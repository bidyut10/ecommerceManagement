const userModel = require("../model/userModel"); //requiring model
...
let { isValidRequestBody, isValid, isValidEmail, isvalidPincode,
       isValidPassword, isValidPhone, isValidImg } = require("../validator/validation");
...
const createUser = async (req, res) => {
  try {
    ...
    if (!isValid(fname)) {
      return res
        .status(400)
        .send({ status: false, message: "Provide the First Name Feild" });
    }
    ...
    if (!isValid(lname)) {
      return res
        .status(400)
        .send({ status: false, message: "Provide the Last Name Feild" });
    }
    ...
    if (!isValid(phone)) {
      return res.status(400).send({
        status: false,
        message:
          "Phone Number Feild is Required",
      });
    }
    ...
    if (PhoneCheck) {
      return res
        .status(400)
        .send({ status: false, message: `This No ${phone} is Already Registered` });
    }
    ...
    //address validation
    if (address) {
      let objAddress = JSON.parse(address); //convert text into a JavaScript object
...
const loginUser = async function (req, res) {
  try {
    ...
    if (!isValidPassword(password)) {
      return res.status(400).send({
        status: false,
        message: "Password Length should be in 8-15 chars only ",
      });
    }
      ...
      if (!isValid(phone) || !isValidPhone(phone)) {
        return res
          ...
          .send({
            status: false,
            message:
              "Please Provide valid Indian Phone Number",
          });
      }
      let PhoneCheck = await userModel.findOne({ phone: phone.trim() });
      if (PhoneCheck) {
